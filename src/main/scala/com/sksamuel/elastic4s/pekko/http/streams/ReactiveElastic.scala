...
object ReactiveElastic {
  ...
  implicit class ReactiveElastic(client: ElasticClient) {
    ...
    def publisher(q: SearchRequest)(implicit actorRefFactory: ActorRefFactory, executionContext: ExecutionContext): ScrollPublisher =
      publisher(q, Long.MaxValue)

    def publisher(q: SearchRequest, elements: Long)(implicit actorRefFactory: ActorRefFactory, executionContext: ExecutionContext): ScrollPublisher =
